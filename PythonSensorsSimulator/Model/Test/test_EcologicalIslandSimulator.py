...
class TestEcologicalIslandSimulator(unittest.TestCase):
    ...
    def test_generate_measure(self):
        ...
        self.assertTrue(
            EcologicalIslandSimulator._EcologicalIslandSimulator__min_fill_percentage <= self.simulator._misurazione <= EcologicalIslandSimulator._EcologicalIslandSimulator__max_fill_percentage)
        self.assertTrue(
            old_measure - EcologicalIslandSimulator._EcologicalIslandSimulator__fill_rate <= self.simulator._misurazione <= old_measure + EcologicalIslandSimulator._EcologicalIslandSimulator__fill_rate)

    def test_simulate(self):
        misurazione = self.simulator.simulate()
        self.assertIsInstance(misurazione, Misurazione)
        self.assertTrue(
            EcologicalIslandSimulator._EcologicalIslandSimulator__min_fill_percentage <= misurazione.get_value() <= EcologicalIslandSimulator._EcologicalIslandSimulator__max_fill_percentage)
