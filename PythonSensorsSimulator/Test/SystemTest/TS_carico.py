...
async def test_RV1(clickhouse_client):
    ...
    try:
        ...
        temp_writers = composite_writer().add_kafka_confluent_writer("temperature", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        umd_writers = composite_writer().add_kafka_confluent_writer("humidity", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        chS_writers = composite_writer().add_kafka_confluent_writer("chargingStation", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        ecoIs_writers = composite_writer().add_kafka_confluent_writer("ecoIslands", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        waPr_writers = composite_writer().add_kafka_confluent_writer("waterPresence", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        dust_writers = composite_writer().add_kafka_confluent_writer("dust_PM10", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        eletricalFault_writers = composite_writer().add_kafka_confluent_writer("electricalFault", KAFKA_HOST, KAFKA_PORT, "http://schema_registry:8081").add_list_writer(list_measure)
        symExecAggregator = simulator_executor_factory()
        num_messages = 1
        num_sensor_per_type = 10
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_temperature_sensor(45.4065, 11.8793, "Test_cell_carico"), temp_writers, 0.01,num_messages)
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_humidity_sensor(45.4068, 11.8794, "Test_cell_carico"), umd_writers, 0.01,num_messages)
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_charging_station_sensor(45.4059, 11.8785, "Test_cell_carico"), chS_writers, 0.01,num_messages)
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_ecological_island_sensor(45.4045, 11.8797, "Test_cell_carico"), ecoIs_writers, 0.01,num_messages)
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_water_presence_sensor(45.4070, 11.8805, "Test_cell_carico"), waPr_writers, 0.01,num_messages)
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_dust_PM10_sensor(45.4069, 11.8800, "Test_cell_carico"), dust_writers, 0.01,num_messages)
        for i in range(num_sensor_per_type):
            symExecAggregator.add_simulator(sensor_factory.create_eletrical_fault_sensor(45.4056, 11.8788, "Test_cell_carico"), eletricalFault_writers, 0.01,num_messages)
